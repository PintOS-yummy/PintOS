																															 ...
																															 uint64_t a3_, uint64_t a4_, uint64_t a5_, uint64_t a6_)
{
	...
	*/
	...
	__asm __volatile(
			...
			"mov %7, %%r9\n"
			"syscall\n"
			: "=a"(ret) // 명령어 실행 후 'rax' 레지스터 값을 'ret' 변수에 저장, 시스템 콜의 결과값
			: "g"(num), "g"(a1), "g"(a2), "g"(a3), "g"(a4), "g"(a5), "g"(a6) // 각각의 C 함수 인자를 어셈블리 코드의 입력으로 제공
			: "cc", "memory"); // 명령어 실행이 조건 코드(cc)와 메모리 상태에 영향을 미칠 수 있음을 나타냄
	return ret;
}

/* Invokes syscall NUMBER, passing no arguments, and returns the
	 return value as an `int'. */
/* 시스템 콜 번호 NUMBER를 호출하고, 인자 없이, 반환값을 `int`로 반환합니다. */
#define syscall0(NUMBER) ( \
		syscall(((uint64_t)NUMBER), 0, 0, 0, 0, 0, 0))

/* Invokes syscall NUMBER, passing argument ARG0, and returns the
	 return value as an `int'. */
/* 시스템 콜 번호 NUMBER를 호출하고, 인자 ARG0를 전달하고, 반환값을 `int`로 반환합니다. */
#define syscall1(NUMBER, ARG0) ( \
		syscall(((uint64_t)NUMBER),  \
						((uint64_t)ARG0), 0, 0, 0, 0, 0))
/* Invokes syscall NUMBER, passing arguments ARG0 and ARG1, and
	 returns the return value as an `int'. */
/* 시스템 콜 번호 NUMBER를 호출하고, 인자 ARG0와 ARG1을 전달하고, 반환값을 `int`로 반환합니다. */
...
/* 시스템 콜 번호 NUMBER를 호출하고, 인자 ARG0, ARG1, ARG2를 전달하고, 반환값을 `int`로 반환합니다. */
...
/*
	...
	피호출자(callee) 저장 레지스터인 %RBX, %RSP, %RBP와 %R12 ~ %R15를 제외한
	레지스터 값을 복제할 필요가 없습니다.
...
/*
	...
	Create child process and execute program corresponds to file on it.
*/
int 
