...
/* PAGE_CNT 연속된 빈 페이지 그룹을 가져와 반환합니다.
PAL_USER가 설정되면, 사용자 풀에서 페이지를 가져옵니다, 그렇지 않으면 커널 풀에서 가져옵니다. 
FLAGS에 PAL_ZERO가 설정되어 있으면, 이면 페이지가 0으로 채워집니다.  
사용 가능한 페이지가 너무 적으면 사용 가능한 페이지가 너무 적으면, PAL_ASSERT가 이 경우 커널이 패닉에 빠집니다. */
void *
...
사용 가능한 페이지가 없으면, PAL_ASSERT가 FLAGS에 설정되어 있지 않는 한, 널 포인터를 반환합니다(이 경우 커널이 패닉에 빠집니다). */
